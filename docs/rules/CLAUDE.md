# Claude AI Rules - Directory of Podiatrists

> **Instrucciones para Claude AI** (Web, Desktop, API)

## 🎯 Proyecto

**Monorepo:** Directorio de podólogos
- Frontend: Next.js 16 + React 19 + TypeScript + Tailwind
- Backend: NestJS 11 + Prisma 6 + PostgreSQL
- Package Manager: pnpm workspaces

## 📚 Docs Obligatorias (LEER PRIMERO)

**Antes de implementar:**
1. [`docs/ai/rules.md`](./docs/ai/rules.md) - Reglas proyecto
2. [`docs/ai/context.md`](./docs/ai/context.md) - Contexto actual
3. [`docs/ai/conventions.md`](./docs/ai/conventions.md) - Convenciones código
4. [`docs/plans/README.md`](./docs/plans/README.md) - Planes desarrollo

**Para features:**
- Lee plan en `/docs/plans/0X-[nombre].md` completamente
- Cuestiona decisiones subóptimas
- Propón alternativas mejores
- Actualiza con decisiones tomadas

## 🏗️ Arquitectura

**Backend (NestJS):**
- Modular (feature = módulo)
- Prisma ORM
- JWT en cookies HTTP-only (NO localStorage)
- DTOs con `class-validator`
- `JwtAuthGuard` global + `@Public()` decorator

**Frontend (Next.js):**
- Server Components por defecto
- TypeScript strict
- Tailwind CSS (NO inline styles)
- Context API para estado
- Servicios en `/client/lib/services/`

## 🚫 Nunca / ✅ Siempre

**❌ NUNCA:**
JWT en localStorage | `any`/`unknown` | Commitear `.env` | Saltarse validación | Estilos inline | Implementar sin leer plan | Commits con refs a IA | Commits "hero"

**✅ SIEMPRE:**
Leer docs | Cuestionar decisiones | Proponer alternativas | Actualizar planes | Type-safe | Validar (cliente + servidor) | Manejar errores | Tests 85% | Actualizar docs | Commits agrupados

## 📁 Estructura

```
directory_of_podiatrists/
├── server/src/          # Backend NestJS
│   ├── auth/           # Autenticación
│   ├── practitioners/  # Profesionales
│   └── prisma/         # DB service
├── client/             # Frontend Next.js
│   ├── app/           # App Router
│   ├── components/    # Components
│   ├── lib/services/  # API services
│   └── contexts/      # Context API
└── docs/              # Documentación
    ├── plans/         # Planes
    ├── project/       # Proyecto
    └── ai/            # Config IA
```

## 🔧 Comandos

```bash
pnpm install                        # Instalar
pnpm run dev                        # Dev completo
pnpm --filter server run dev:watch  # Backend
pnpm --filter client run dev        # Frontend

cd server
pnpm prisma migrate dev             # Migraciones
pnpm prisma db seed                 # Seed

pnpm --filter client run test       # Tests frontend
pnpm --filter server run test:e2e   # Tests e2e
```

## 🤔 Proceso

1. Leer plan en `/docs/plans/`
2. Entender contexto
3. Revisar solución
4. Cuestionar si subóptimo
5. Proponer alternativas
6. Discutir si incertidumbre
7. Documentar decisiones
8. Implementar
9. Actualizar plan
10. Tests (85% coverage)
11. Commit

## 🎨 Convenciones

- Interfaces (NO `type` sin razón)
- async/await (NO callbacks)
- const por defecto
- Arrow functions
- Template literals
- Optional chaining (`?.`) y nullish coalescing (`??`)

**Naming:**
- Componentes: `PascalCase`
- Funciones: `camelCase`
- Constantes: `UPPER_SNAKE_CASE`
- Utils: `kebab-case`

## 🔐 Seguridad

- JWT en cookies HTTP-only con `sameSite: 'strict'`
- Validación cliente + servidor
- Prisma queries parametrizadas
- CORS configurado
- NUNCA commitear secrets
- HTTPS en producción

## 📝 Commits

**Formato:** `type(scope): subject`

**Tipos:** `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`

**Ejemplos:**
```bash
feat(auth): add password reset
fix(search): improve performance
docs(plans): update auth plan
test(practitioners): add e2e tests
```

**❌ NO:**
```bash
feat: implement entire system  # Hero
feat(auth): add login (co-authored with Claude)  # Ref IA
fix: stuff  # Vago
```

## 📋 Steps Development

**1. Pre-implementación:**
- ⬜ Leer plan `/docs/plans/0X-[nombre].md`
- ⬜ Entender contexto
- ⬜ Cuestionar approach

**2. Implementación:**
- ⬜ Seguir convenciones
- ⬜ Validar cliente + servidor
- ⬜ Manejar errores

**3. Tests (OBLIGATORIO):**
- ⬜ Tests unitarios
- ⬜ Tests e2e
- ⬜ **85% coverage mínimo**
- ⬜ **Step NO completa si tests no pasan**

**4. Post-implementación:**
- ⬜ Actualizar plan
- ⬜ Actualizar `/docs/plans/README.md`
- ⬜ Commit
- ⬜ Marcar completado

**Actualizar docs:**
```bash
# 1. Actualizar plan (marcar tasks, aprendizajes)
# 2. Actualizar /docs/plans/README.md
# 3. Commit
git add docs/plans/
git commit -m "docs(plans): mark step 0X as completed"
```

## 🔄 Sincronización

**IMPORTANTE:** Sincronizar con `.cursorrules`

Cuando actualices `CLAUDE.md` → actualiza `.cursorrules` (y viceversa)

## 📖 Recursos

- [Docs completa](./docs/README.md)
- [Planes](./docs/plans/README.md)
- [Guía IA](./docs/AI_COLLABORATION_GUIDE.md)
- [Arquitectura](./docs/project/architecture.md)

### Leyenda de Estados

- ⬜ Pendiente
- 🔄 En progreso
- ✅ Completado
